...
you are connected with. Updates are only published to the network if the
Upstream client is running.
...
To contribute to another Upstream user's project, you must first find it on the
Radicle network. To find a project, you need its [**Radicle ID**][ri]. This can
be found in the project page under the [project name][pn], or in a seed node UI
(i.e. [seedling.radicle.xyz][sn]).
...
[remote][re] for that project which is uniquely linked to your Device ID. You
can view this remote in the manage remotes modal.

![Peer switcher][ps]

Once your remote is created for a project, it will also move the project to your
**Projects** tab.
...
find this in the `Manage Remotes` modal) and `<project-id> is the Radicle ID of
the project you wish to clone (you can find this on the project page).

## Getting changes from a project

### Adding remotes
To view and pull changes from another Upstream user's version of your project,
you must first **add their remote**. To do this, you must add their **Device
ID** to the list of remotes that you follow.
...
changes from a peer we follow. Assuming that your network of peers and seeds
also track the peer you are looking for, you will get their changes:

1. On application startup, where your peer will ask for any new changes from all
   the peers you follow. For more information on following, [click here][fo].
...
Where `<remote>` is of the following form: `<display name>@<Device ID>/<branch>`
...
[fo]: understanding-radicle/how-it-works.md/#tracking
